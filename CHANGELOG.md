...
- 安装改成纯拖拽: 下 `.dmg` -> 拖进「应用程序」-> 打开 app 点一下「好」, 终端命令自动装上, 全程不用开终端
- 首次打开会被 macOS 拦一次 (这个 app 没买 Apple 开发者签名): 系统设置 → 隐私与安全性 → 「仍要打开」放行一次即可, 之后不再提示

### Removed

- 一键安装命令 (`curl ... | bash`) 与 `.tar.gz` 安装包: 发布页只留 dmg; 存了那条命令的改用上面的装法
...
- macOS 预编译安装包 (Apple Silicon / Intel 通用): 从 Releases 直接下载, 或一条 curl 命令装最新版, 不再需要自己从源码构建
- 每次发版自动出包并附校验和; 升级 = 重跑同一条安装命令

### Changed

- 无 Apple 开发者签名, 但按上述方式安装不会被系统拦下: 不用去「隐私与安全性」点「仍要打开」

## [0.11.0] - 2026-08-25

### Changed

- 命令行工具搬进 app 包体, 终端里的 `jj-agentic-proxy` 变成指过去的快捷方式: 以后升级只装 app 一步到位, 不会再出现命令行和 app 版本不一致
- 装完提示执行一次 `jj-agentic-proxy start` 切到新版本; 首次切换时系统可能再问一次「是否允许接受传入网络连接」, 选允许 (拒绝的表现是局域网连不上而本机正常)
- 安装失败时终端命令会一起失效 (只有一份副本, 在 app 里); 重跑安装即恢复

### Removed

- 安装脚本去掉 `--cli-only`: 只装命令行已无意义 (它就在 app 包体里)
